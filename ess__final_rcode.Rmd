...
glm = glm(data = task3, response ~ age + sex_mf, family = binomial, data = self_cons) 
...
age <- rep(seq(from = 0, to = 100), 2) #create sequence of ages 0 to 100 for both male and female
f <- rep("Female", 101) 
m <- rep("Male", 101) 
mf <- c(f,m)
...
probs <- predict(glm, newdata = age_mf, type = "response", se.fit = TRUE)

graph1 <- data.frame(age_mf, probs$fit, probs$se.fit)
colnames(graph1) <- c("Age", "Sex", "Probability", "SE")
graph1

predicted_probs <- predict(glm, newdata = age_mf, type = "response", se.fit = TRUE)
